...
/// Clickable regions of the header / message box, from the last draw.
#[derive(Clone, Copy, Default)]
struct ClickZones {
...
impl PaneCtl {
    ...
    /// Set or clear the pane label. `None` sends JSON null (`--clear`).
...
impl App {
    ...
    /// Push identity tokens. No border title — the header already names
    /// the view, and the token identifies the pane.
    ...
    pub fn draw(&mut self, frame: &mut Frame) {
        ...
        let [header, message, button, sync, list, footer] = Layout::vertical([
            Constraint::Length(1),
    ...
    fn hints(&self) -> Vec<(&'static str, &'static str)> {
        let hints: Vec<(&'static str, &'static str)> = vec![
            ("⏎", "stage"),
            ("a", "all"),
            ("u", "none"),
            ("c", "msg"),
