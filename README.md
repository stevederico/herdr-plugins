...
- **space-title** names spaces from the status emoji plus project folder and puts the Grok/agent session title on the subtitle. If the pane still sits in `~/Projects` (or `~`), it infers the git repo the session is actually in. Pins the Grok session UUID (`grok-resume` resumes it after a pane reset)
